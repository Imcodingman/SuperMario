...
用原生 HTML5 Canvas + JavaScript 写的横版闯关小游戏，分两个世界，一共七关。没有依赖，也不用构建，手机和电脑浏览器都能玩。
...
每关抓完旗杆（城堡关是碰到斧头）会直接进入下一关，分数、金币、剩余命数和变大状态都会带过去。世界 2 比世界 1 难：倒计时只有 300，敌人走得更快，还有新的机关。
...
| 2-2 | 终极城堡（最终关） | 转得更快更长的火焰棍、从岩浆里跳出来的火球、岩浆上的移动平台、炮台；魔王喷火更频繁，一次喷两颗 |
...
- **岩浆火球**（2-2）：隔一会儿从岩浆里跳出来再落回去，看准时机再跳过岩浆坑
...
```js
{
  ...
  boss: { x: 186, min: 181, max: 190, fireRate: 80, double: true, hop: 70 },
                             // 可选，魔王；有魔王的关卡用斧头代替旗杆
                             // fireRate 喷火间隔（帧），double 一次喷两颗，hop 跳跃间隔
  build() {                  // 用下面的小工具搭地形
