...
用原生 HTML5 Canvas + JavaScript 写的横版闯关小游戏，分两个世界，一共九关。没有依赖，也不用构建，手机和电脑浏览器都能玩。
...
每关抓完旗杆（城堡关是碰到斧头）会直接进入下一关，分数、金币、剩余命数和变大状态都会带过去。世界 2 比世界 1 难：倒计时只有 300，敌人走得更快，还有新的机关，而且越往后越难。
...
| 2-4 | 熔岩堡垒（最终关） | 站上去就往下掉的落下平台、双头火焰棍、几乎连成片的岩浆坑；魔王走得更快、喷火和跳跃都更频繁 |
...
- **魔王**（1-5、2-2）：在吊桥上来回走动、跳跃，还会朝你喷火球，踩不死。从它头上跳过去或趁它跳起时从下面钻过去，碰到斧头后吊桥断掉，魔王掉进岩浆。2-2 的魔王喷火更勤，一颗追着你的高度飞，另一颗贴着吊桥平飞
...
- **落下平台**（2-4）：灰白色的平台，站上去一下就开始往下掉，要赶紧跳到下一块；普通的橙色平台不会掉
- **双头火焰棍**（2-4）：同一个中心伸出两根火焰棍，像螺旋桨一样转，空档更小
...
```js
{
  ...
  enemies: [[22, 12], [30, 12, 'koopa']],  // 敌人 [列, 站立行, 类型]：不写是栗子怪，'koopa' 乌龟，'para' 飞龟
  platforms: [...],          // 可选，平台：axis 'x' 左右移动、'y' 上下移动、'fall' 落下平台
  theme: 'snow',             // 可选，冰雪主题（配合 snow: true 飘雪、ice: true 冰面打滑）
  ...
  boss: { x: 186, min: 181, max: 190, fireRate: 80, double: true, hop: 70, speed: 0.8 },
                             // 可选，魔王；有魔王的关卡用斧头代替旗杆
                             // fireRate 喷火间隔（帧），double 一次喷两颗，hop 跳跃间隔，speed 走路速度
  build() {                  // 用下面的小工具搭地形
